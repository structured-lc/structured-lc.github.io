...
### Flashcard
For each prime nums[i], find minimum x where x OR (x+1) = nums[i]; for even primes (only 2), no solution exists; for odd primes, brute-force or use bit-pattern analysis.
