...
### Flashcard
Use matrix exponentiation to track character frequency evolution; each transformation step is a linear operation on the frequency vector, so compute matrix^t in O(log t).
