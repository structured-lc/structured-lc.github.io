...
### Flashcard
Use recursive DFS to check if both nodes are null (match), only one is null (not identical), or values differ (not identical), then recurse on left and right subtrees.
